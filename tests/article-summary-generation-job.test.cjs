...
} = require('../src/backend/article-summary-generation-job.cjs');
const {
...
function insertArticleContent(
  db,
  {
    ...
    publishedAt = null,
    viewCount = null,
    fetchedAt = '2026-03-30T18:45:00.000Z',
  },
) {
  db.prepare(`
...
test('runArticleSummaryGenerationJob summarizes completed articles without duplicating target article pairs', async () => {
  ...
  insertArticleContent(db, {
    ...
    bodyText: 'This article should not be summarized yet.',
  });
  ...
  const summaryCalls = [];
  const result = await runArticleSummaryGenerationJob({
    db,
    now: () => '2026-03-30T19:00:00.000Z',
    createId: createIdGenerator('analysis-1'),
    generateArticleSummary: async ({ monitoringTarget, article }) => {
      summaryCalls.push({ monitoringTarget, article });

      return {
        summary:
          'Acme Holdings faces a labor dispute. Governance scrutiny is rising. Leadership response is pending.',
  ...
  const savedAnalysis = db
    .prepare(`
      SELECT id, relevance_score, topic_labels, summary, risk_score, rationale, model_version, summary_generated_at, updated_at
  ...
  assert.deepEqual(normalizeRow(savedAnalysis), {
    ...
    summary_generated_at: '2026-03-30T19:00:00.000Z',
    updated_at: '2026-03-30T19:00:00.000Z',
  });
    ...
    processedAnalyses: [
      {
        ...
        summary:
          'Acme Holdings faces a labor dispute. Governance scrutiny is rising. Leadership response is pending.',
        modelVersion: 'gpt-5.4-mini',
        summaryGeneratedAt: '2026-03-30T19:00:00.000Z',
      },
    ],
    totalProcessed: 1,
    summarizedAnalyses: 1,
  });

  db.close();
});

test('runArticleSummaryGenerationJob replaces existing summaries only when a reanalysis request is supplied', async () => {
  const db = createDatabase();
  ...
  const skippedResult = await runArticleSummaryGenerationJob({
    db,
    generateArticleSummary: async () => {
      throw new Error('generateArticleSummary should not be called without a reanalysis request');
    },
  });
  const skippedAnalysis = db
    .prepare(`
      SELECT id, summary, model_version, summary_generated_at, updated_at
      FROM article_analysis
      WHERE workspace_id = ? AND monitoring_target_id = ? AND article_id = ?
    `)
    .get('workspace-1', 'target-1', 'article-1');
  ...
  const summaryCalls = [];
  const reanalysisResult = await runArticleSummaryGenerationJob({
    db,
    now: () => '2026-03-30T19:30:00.000Z',
    createId: () => {
    ...
    generateArticleSummary: async ({ monitoringTarget, article }) => {
      ...
      return {
        summary:
          'Acme Holdings is facing a governance probe. Labor complaints are part of the reported trigger.',
  ...
  assert.deepEqual(normalizeRow(savedAnalysis), {
    ...
    summary_generated_at: '2026-03-30T19:30:00.000Z',
    risk_scored_at: '2026-03-30T12:20:00.000Z',
    updated_at: '2026-03-30T19:30:00.000Z',
  });
    ...
    processedAnalyses: [
      {
        ...
        summary:
          'Acme Holdings is facing a governance probe. Labor complaints are part of the reported trigger.',
        modelVersion: 'gpt-5.4',
        summaryGeneratedAt: '2026-03-30T19:30:00.000Z',
      },
    ],
    totalProcessed: 1,
    summarizedAnalyses: 1,
...
test('runArticleSummaryGenerationJob validates generator output', async () => {
  const db = createDatabase();
  ...
  await assert.rejects(
    () =>
      runArticleSummaryGenerationJob({
        ...
        generateArticleSummary: async () => ({
          summary:
            'Sentence one. Sentence two. Sentence three. Sentence four should fail validation.',
          modelVersion: 'gpt-5.4-mini',
        }),
      }),
    (error) =>
      error instanceof ArticleSummaryGenerationJobError &&
      error.code === 'INVALID_INPUT' &&
      /summary must contain at most 3 sentences/u.test(error.message),
  );
